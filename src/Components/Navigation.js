    ...
    render() {
        return (
            <div className="nav-wrapper">
                <img className="mobile-nav-closed" onClick={this.mToggle} id="mobileNavClosed"
                     src={require('./Assets/menu-' + this.state.color + '.png')} alt="menu-icon"/>
                ...
                <nav className="nav-large-screen">
                    <div className="mobile-nav-header">
                        <Link id="designing-water-header-text-mobile-nav" to="/"
                              style={{whiteSpace: 'nowrap', color: "white"}}>Designing Water</Link>
                        <img className="mobile-nav-close" onClick={this.mClose} id="mobileNavClose"
                             src={require('./Assets/close.png')} alt="close-menu" style={{zIndex: '3'}}/>
                    </div>
                    <ul>       {/* Props here are used to append the class name. This is so each page can add its own colors to each menu link */}
                        <li><Link className={"header-link to-schedule-" + this.props.pageClass}
                                  to="/schedule" /*style={{color: this.state.color}}*/>Schedule</Link></li>
                        <li><Link className={"header-link to-speakers-" + this.props.pageClass}
                                  to="/speakers" /*style={{color: this.state.color}}*/>Speakers</Link></li>
                        <li><Link className={"header-link to-venue-" + this.props.pageClass}
                                  to="/venue" /*style={{color: this.state.color}}*/>Venue</Link></li>
                        <li><Link className={"header-link to-register-" + this.props.pageClass}
